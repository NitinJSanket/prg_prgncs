...
from Network.NetworkIdentity import CIFAR10Identity
...
def GenerateBatch(DirNamesTrain, TrainLabels, ImageSize, MiniBatchSize, PerEpochCounter):
    ...
    while ImageNum < MiniBatchSize:
        ...
        Label = np.ones(OutSize)#[0:8,0:8#convertToOneHot(TrainLabels[RandIdx], 10)
...
def TrainOperation(ImgPH, LabelPH, DirNamesTrain, DirNamesTest, TrainLabels, TestLabels, NumTrainSamples, ImageSize,
    ...
    prImg = CIFAR10Identity(ImgPH, ImageSize, MiniBatchSize)

    with tf.name_scope('Loss'):
        # Cross-Entropy Loss
        # 10 is used because of 10 classes
        loss = tf.reduce_mean(prImg-LabelPH)
...
def main():
    ...
    LabelPH = tf.placeholder(tf.float32, shape=(MiniBatchSize, OutSize[0], OutSize[1], OutSize[2]), name='Label')
