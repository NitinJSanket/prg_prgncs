...
def model(img, layers, UpSample):
    ...
    if UpSample:
      ...
      for filters in layers:
          ...
          print(net.shape)
      net = Conv(inputs = net, filters = 4, kernel_size = [3,3], strides = (1,1), padding = "same")
...
def main(args):
  ...
  # output_ = model_(x)
  output_ = model(x, layers, args.upsample)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-s','--Shape', type=str, default="[1, 512, 256, 1]", help='Scaling of input dimention')
  ...
  parser.add_argument('-u','--upsample', type=bool, default=False, help="Upsampling of the network")
