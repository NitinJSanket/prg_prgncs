...
# Clean comments
# Test print add space before number
...
def TrainOperation(ImgPH, LabelPH, DirNamesTrain, DirNamesTest, TrainLabels, TestLabels, NumTrainSamples, ImageSize,
    ...
    prLogitsT, _ = CIFAR10Teacher(ImgPH, ImageSize, MiniBatchSize)
    prLogitsS, _ = CIFAR10Student(ImgPH, ImageSize, MiniBatchSize)

    with tf.name_scope('Loss'):
        # Weight factors for each of the loss components
        Lambda = [1.0, 0.1, 0.01]
        # Normalize the weight factors
        Lambda = Lambda/np.sum(Lambda)
        lossT = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prLogitsT, labels=LabelPH))
        lossS = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prLogitsS, labels=LabelPH))
        prLogitsTOneHot = tf.one_hot(tf.argmax(prLogitsT, dimension = 1), depth = 10)
        prLogitsSOneHot = tf.one_hot(tf.argmax(prLogitsS, dimension = 1), depth = 10)
        lossProj = 0.5*(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prLogitsS, labels=prLogitsTOneHot)) + 
                        tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=prLogitsT, labels=prLogitsSOneHot)))
        # Cross-Entropy Loss
        # 10 is used because of 10 classes
        # Total Loss
        loss = Lambda[0]*lossT + Lambda[1]*lossS + Lambda[2]*lossProj
...
def main():
    ...
    Parser.add_argument('--BasePath', default='/home/nitin/Research/SpectralCompression/CIFAR10', help='Base path of images, Default:/media/nitin/Research/Homing/DataMapping/AllFrames')
    Parser.add_argument('--NumEpochs', type=int, default=200, help='Number of Epochs to Train for, Default:200')
    Parser.add_argument('--DivTrain', type=int, default=1, help='Factor to reduce Train data by per epoch, Default:1')
    Parser.add_argument('--MiniBatchSize', type=int, default=1000, help='Size of the MiniBatch to use, Default:48')
    ...
    ImgPH = tf.placeholder(tf.float32, shape=(MiniBatchSize, ImageSize[0], ImageSize[1], 3))
